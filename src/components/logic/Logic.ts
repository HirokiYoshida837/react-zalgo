...
import {IRandomGenerator, IRandomGeneratorFactory} from "./random/IRandomGenerator";
...
export const Glitch = (body: string, params?: GlitchParams) => {
  ...
  const randomGenerator: IRandomGenerator = params?.randomGeneratorFactory?.create() ?? DefaultRandomGeneratorFactory.create();
